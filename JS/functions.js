...
import { renderMap2D, renderTiles,renderTileGround,  renderPlayer2D,screenBlanking2D} from "./Render/render2d.js";
...
for (let i = LARG_TILE; i < LARG_TILE * 9; i += LARG_TILE) {
  for (let j = ALT_TILE; j < ALT_TILE * 9; j += ALT_TILE) {
    ...
    if (!gameMap.existingTilePosCheck(t.pos)) {
      gameMap.addGround(new Tile(i, j, true)); // Adicionando tile de chão
    }
  }
}
}
